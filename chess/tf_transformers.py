...
def create_padding_mask(seq, add_dimension = True, pad_token = 0):
    seq = tf.cast(tf.math.equal(seq, pad_token), seq.dtype)
...
def scaled_dot_product_attention(q, k, v, mask):
  ...
  # add up to 1.
    attention_weights = tf.keras.layers.Softmax(axis=-1)(scaled_attention_logits) #tf.nn.softmax(scaled_attention_logits, axis=-1)  # (..., seq_len_q, seq_len_k)
...
class CustomSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    def __init__(self, d_model, warmup_steps=4000, temperature = 10):
        ...
        self.d_model = d_model
        self.d_model = tf.cast(self.d_model, x.dtype)
