...
def load_real_samples(list_files):
    # load dataset
    X = list(np.zeros(len(list_files)))
    count = 0
    print(len(list_files))
    for elt in range(len(list_files)):
#        if list_files[elt] in os.listdir('./image_clean_128'):
        path = './image_clean_128'+'/'+list_files[elt]
        img = imageio.imread(path)
        X[elt] = np.array(img).astype('float32')
        if count%1000 == 0:
            print(count)
        count += 1
...
def summarize_performance(status, g_model, latent_dim, n_samples=25):
	...
	for i in range(n_samples):
		...
		pyplot.imshow(X[i])
	# save plot to file
	filename1 = './images_gen/plot_%s.png' % (name)
	pyplot.savefig(filename1)
	pyplot.close()
	# save the generator model
	filename2 = './model/model_%s.h5' % (name)
